...
@ApiTags('Location Manager')
@Controller('location-manager')
export class LocationManagerController {
    constructor(
    ...
    async create(@Request() req, @Response() res, @Body() body: RegistrationDTO) {
        try {
            const dispatcher = await this.locationMgrService.create(body)
    ...
    async findAll(@Request() req, @Response() res) {
        try {
            await this.locationMgrService.getAll(req.user.roles);
    ...
    async findById(@Request() req, @Response() res, @Param('id') id: string) {
        try {
            await this.locationMgrService.getOne(id, req.user.roles);
